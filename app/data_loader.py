...
def compare_players(dat,players:list,seasons:list,transpose:str):
    
    grouped = dat[(dat.player_name.isin(players)) & (dat.season.isin(seasons)) ].groupby(["player_name","season"])
    
    rows = []
    for (player_name, season), group in grouped:
        dribbles_attempts = group["dribbles_attempts"].sum()
        duels_total = group["duels_total"].sum()
        shots_on = group["shots_on"].sum()
        games_count = group["fixture_id"].count()
        ...
        row = {
            ...
            # Per 90 stats
            "goals_converted_per_shot_target_per_90": group["goals_total"].sum() / shots_on * 90 if shots_on >= 7 else np.nan,
            "total_shots_per_90": group["shots_total"].sum() / group["games_minutes"].sum() * 90 if group["games_minutes"].sum() > 0 else 0,
            "shots_on_target_per_90": shots_on / group["games_minutes"].sum() * 90 if group["games_minutes"].sum() > 0 else 0,
            ...
            "attempted_dribbles_per_90": dribbles_attempts / group["games_minutes"].sum() * 90 if group["games_minutes"].sum() > 0 else 0,
            ...
            "dribble_success_rate_per_90": group["dribbles_success"].sum() / dribbles_attempts if dribbles_attempts >= 7 else np.nan,
            ...
            "average_pass_accuracy_per_90": (group["pass_accuracy_perc"] * group["games_minutes"]).sum()/group["games_minutes"].sum() if games_count >= 7 else np.nan,
            ...
            "duels_contested_per_90": duels_total / group["games_minutes"].sum() * 90 if group["games_minutes"].sum() > 0 else 0,
            "duels_won_per_90": group["duels_won"].sum() / group["games_minutes"].sum() * 90 if group["games_minutes"].sum() > 0 else 0,
            "duels_won_percentage_per_90": group["duels_won"].sum() / duels_total if duels_total >= 7 else np.nan,
    ...
    final_dat = pd.DataFrame(rows)

    if transpose:
        final_dat = final_dat.set_index('player_name').T
...
def calculate_fixture_stats(dat,group_cols = []):
    ...
    merged_data['draw'] = merged_data['draw'].apply(lambda x: x[0])
    merged_data['loss'] = merged_data['loss'].apply(lambda x: x[0])
    merged_data['goal_diff'] = merged_data['team_goals_scored'] - merged_data['team_goals_conceded']

    # Add dates:
    fixture_date = dat[['fixture_id','week_e','year_e','season','fixture_date']].drop_duplicates()
    merged_data = pd.merge(merged_data,fixture_date[['fixture_id','week_e','year_e','season','fixture_date']],how = 'left',on = 'fixture_id')
